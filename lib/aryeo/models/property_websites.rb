...
module Aryeo
  ...
  class PropertyWebsites
    ...
    def list_invalid_properties
      ...
      if @branded_url.to_s.length > 65535
        invalid_properties.push('invalid value for "branded_url", the character length must be smaller than or equal to 65535.')
      end

      if @branded_url.to_s.length < 1
        invalid_properties.push('invalid value for "branded_url", the character length must be great than or equal to 1.')
      end

      if @unbranded_url.nil?
        invalid_properties.push('invalid value for "unbranded_url", unbranded_url cannot be nil.')
      end

      if @unbranded_url.to_s.length > 65535
        invalid_properties.push('invalid value for "unbranded_url", the character length must be smaller than or equal to 65535.')
      end

      if @unbranded_url.to_s.length < 1
    ...
    def branded_url=(branded_url)
      if branded_url.nil?
        fail ArgumentError, 'branded_url cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] unbranded_url Value to be assigned
    def unbranded_url=(unbranded_url)
      if unbranded_url.nil?
        fail ArgumentError, 'unbranded_url cannot be nil'
      end

      if unbranded_url.to_s.length > 65535
        fail ArgumentError, 'invalid value for "unbranded_url", the character length must be smaller than or equal to 65535.'
      end

      if unbranded_url.to_s.length < 1
        fail ArgumentError, 'invalid value for "unbranded_url", the character length must be great than or equal to 1.'
      end
