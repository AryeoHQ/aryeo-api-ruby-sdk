  ...
  class Appointment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Aryeo::Appointment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Aryeo::Appointment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@status.nil? && @status.to_s.length > 255
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["SCHEDULED", "UNSCHEDULED", "RESCHEDULED", "CANCELED"])
      return false unless status_validator.valid?(@status)
      return false if !@status.nil? && @status.to_s.length > 255
      ...
      return false if !@start_at.nil? && @start_at.to_s.length > 255
      return false if !@start_at.nil? && @start_at.to_s.length < 0
      return false if !@end_at.nil? && @end_at.to_s.length > 255
      return false if !@end_at.nil? && @end_at.to_s.length < 0
      true
    end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["SCHEDULED", "UNSCHEDULED", "RESCHEDULED", "CANCELED"])
